...
// FONT 변수 중복 선언 방지: 기존 선언이 있다면 제거하고 이 선언만 유지
const FONT = "BNK Gothic, 'Noto Serif KR', 'Noto Sans KR', serif";

// 네비게이션 바(로고) 컴포넌트
function NavBar({ step, onBack }) {
  const [busanBankLogoError, setBusanBankLogoError] = useState(false);
  const [artlogLogoError, setArtlogLogoError] = useState(false);
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        {!busanBankLogoError ? (
          <img
            ...
            onError={() => setBusanBankLogoError(true)}
          />
        ) : (
          <span style={{ fontWeight: 600, fontSize: 16, color: "#1870b7" }}>부산은행</span>
        )}
        {!artlogLogoError ? (
          <img
            src="/artlog-logo.jpg"
            alt="Artlog360 로고"
            style={{ height: 28, objectFit: "contain", display: "block" }}
            onError={() => setArtlogLogoError(true)}
          />
        ) : (
          <span style={{ fontWeight: 700, fontSize: 17, color: "#000" }}>Artlog360</span>
        )}
      </div>
      <div style={{ flex: 1 }} />
      <div style={{ fontSize: 14, color: "#888", fontFamily: FONT }}>
        {step === 1 && "분석 준비"}
        {step === 2 && "AI 분석결과"}
        {step === 3 && "카드뉴스"}
      </div>
...
// 폰트 link 동적삽입
function useBNKGothicFont() {
  useEffect(() => {
    const id = "bnk-gothic-font-link";
    if (document.getElementById(id)) return;
    const link = document.createElement("link");
    link.id = id;
    link.rel = "stylesheet";
    link.href = "https://cdn.jsdelivr.net/gh/webfontworld/bnk/Bnkgothic.css";
